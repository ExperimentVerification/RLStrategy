...
# Always defect for the decision making part
class RLAlwaysDefects(Strategy):
    __QTable4L = {}
    __Counter = 0
    __LastLambda = 0.0
    __CoopRatio = 0.3
    ...
    def __init__(self, game):
        ...
        RLAlwaysDefects.__Counter = 0


    def respond(self, game):
        RLAlwaysDefects.__Counter += 1
        if self.getLastResponsePair() == 0:
            # cooperate for the first round'
            self._lambda.nochange()
            self.__LastAction4L = 0
            RLAlwaysDefects.__LastLambda = round(self._lambda.getValue(), 1)
            return 'D'
        else:
            ...
            if len(self.__mybeforeLastN) < game.RL_memLen: # memory length = n
                ...
                if self.__LastState4L == -999:
                    ...
                    if self.__CurrentState4L not in RLAlwaysDefects.__QTable4L:
                        RLAlwaysDefects.__QTable4L[self.__CurrentState4L] = [0, 0, 0]
    ...
    def learningResult4L(self, OppLastAction, MyLastAction, game):
        ...
        RLAlwaysDefects.__QTable4L[self.__LastState4L][self.__LastAction4L] += game.RL_lR * \
                                                                               (Reward4L + game.RL_disF * RLAlwaysDefects.__QTable4L[self.__CurrentState4L][self.__CurrentAction4L] -
                                                                                RLAlwaysDefects.__QTable4L[self.__LastState4L][self.__LastAction4L])
    ...
    def getReward(self, OppLast, MyLast, GAME):
        valLambda = float(RLAlwaysDefects.__LastLambda)
    ...
    def getBestChange(self, state, game):
        self.__ExplorePercentage = -(5.0 / game.Iter_N) * RLAlwaysDefects.__Counter + 5

        if random.randint(0, 99) < self.__ExplorePercentage:
            return random.randint(0, 2)
        else:
            return RLAlwaysDefects.__QTable4L[state].index(max(RLAlwaysDefects.__QTable4L[state]))
    ...
    def name(self):
        return "RL-based AlwaysDefect - update parameter and then return 'D' as the decision"
